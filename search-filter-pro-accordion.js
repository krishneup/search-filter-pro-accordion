...
function accordionLoader() {
    ...
    jQuery(".searchandfilter>ul>li[data-sf-field-input-type^='checkbox'] h4").wrap("<a class='accordion-title' href='#'></a>");

    // use this if you want to add icon after widget title, use custom css for styling
    jQuery(".searchandfilter>ul>li[data-sf-field-input-type^='checkbox'] h4").append('  <i class="fa fa-chevron-circle-down icon-class" aria-hidden="true"></i>');

    // add class to input field 'ul' 
    jQuery(".searchandfilter>ul>li[data-sf-field-input-type^='checkbox']>ul").addClass("accordion-list");

    var allAccordions = jQuery('.accordion-list').hide();

    //when element is clicked, toggle it for checkbox
    jQuery(".searchandfilter>ul>li[data-sf-field-input-type^='checkbox']>a").click(function () {
