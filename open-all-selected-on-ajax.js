...
function accordionLoader() {
  ...
  jQuery("li[data-sf-field-input-type^='checkbox'] h4").wrap(
    "<a class='accordion-title' href='#'></a>"
  );

  // use this if you want to add icon after widget title, use custom css for styling
  jQuery("li[data-sf-field-input-type^='checkbox'] h4").append(
    '  <i class="fa fa-chevron-circle-down icon-class" aria-hidden="true"></i>'
  );

  // add class to input field 'ul'
  jQuery("li[data-sf-field-input-type^='checkbox'] ul").addClass(
    "accordion-list"
  );
...
jQuery(document).on("click", ".accordion-list", function () {
  ...
  new_array.push(last_saved_accordion);
...
(function (jQuery) {
  ...
  jQuery(document).on("sf:ajaxfinish", ".searchandfilter", function () {
    ...
    uniqueChars.forEach(function (item) {
      //console.log(item);
      jQuery("." + item + " " + "a")
        .next()
        .show();
